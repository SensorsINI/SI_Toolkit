...
from SI_Toolkit.Pytorch.Pytorch_Functions.utilis import (
    get_device,
    set_seed,
    get_net,
    create_full_name,
...
from SI_Toolkit.Pytorch.Pytorch_Functions.Dataset import Dataset
...
def train_network(nni_parameters=None):
    ...
    print('The training sets sizes are: {}'.format(training_dataset.df_lengths))
    ...
    validation_generator = data.DataLoader(dataset=validation_dataset, batch_size=512, shuffle=False, num_workers=a.num_workers)

    # Print parameter count
    print_parameter_count(net)  # Seems not to function well

    # Select Optimizer
    optimizer = optim.Adam(net.parameters(), amsgrad=True, lr=a.lr)
    ...
    # Init Tensorboard
    comment = f' batch_size={a.batch_size} lr={a.lr} exp_len={a.exp_len}'
    tb = SummaryWriter(comment=comment, log_dir=net_info.path_to_net+'runs/')
    ...
    for epoch in range(a.num_epochs):
        ...
        for batch, labels in tqdm(training_generator):  # Iterate through batches
            ...
            net(rnn_input=batch[:a.wash_out_len, :, :])
        ...
        for (batch, labels) in tqdm(validation_generator):
            ...
            loss = criterion(out[:, a.wash_out_len: a.exp_len],
                             labels[:, a.wash_out_len: a.exp_len])
        ...
        dict_history['train_loss'].append(
            train_loss.detach().cpu().numpy() / train_batches / (a.exp_len - a.wash_out_len))
        dict_history['dev_loss'].append(
            dev_loss.detach().cpu().numpy() / dev_batches / (a.exp_len - a.wash_out_len))
