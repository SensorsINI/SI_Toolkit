...
class predictor_ODE_tf_pure:
    ...
    def _predict_tf_tile(self, initial_state, Q, batch_size):  # Predicting multiple control scenarios for the same initial state
    ...
    def _predict_tf(self, initial_state, Q):
        ...
        for k in tf.range(self.horizon):
            next_state = self.next_step_predictor.step(next_state, Q[:, k, :], None)
...
predictor = predictor_ODE_tf_pure(horizon, 0.02, 10)
'''
