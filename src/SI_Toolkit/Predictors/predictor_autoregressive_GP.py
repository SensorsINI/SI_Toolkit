...
class predictor_autoregressive_GP:
    ...
    @Compile
    def predict_tf_m(self, initial_state, Q_seq):
        # initial_state = tf.expand_dims(initial_state, axis=0)  # COMMENT OUT FOR TF MPPI
        self.outputs = tf.TensorArray(tf.float64, size=self.horizon+1, dynamic_size=False)
...
horizon = 35
num_rollouts = 3500
model_name = "SGP_10_RR"
predictor = predictor_autoregressive_GP(model_name, horizon=horizon, num_rollouts=num_rollouts)
