...
class predictor_autoregressive_GP:
    def __init__(self, horizon, num_rollouts=1):
        ...
        self.normalizing_outputs = tf.convert_to_tensor(self.model.norm_info[['angleD', 'angle_cos', 'angle_sin', 'position', 'positionD']], dtype=tf.float64)
        ...
        self.initial_state = tf.random.uniform(shape=[self.num_rollouts, 4], dtype=tf.float32)
    ...
    def predict_tf(self, initial_state, Q_seq):
        ...
        p = tf.gather(self.initial_state, STATE_INDICES['position'], axis=1)

        # s = tf.repeat(s, repeats=self.num_rollouts, axis=0)  # COMMENT OUT FOR TF MPPI
        self.outputs = self.outputs.write(0, tf.concat([s[..., :-1], p[..., tf.newaxis], s[..., -1, tf.newaxis]], axis=1))

        s, p = self.step(s, p, Q_seq[:, 0, :])

        self.outputs = self.outputs.write(1, tf.concat([s[..., :-1], p[..., tf.newaxis], s[..., -1, tf.newaxis]], axis=1))
        for i in tf.range(1, self.horizon):
            # x = tf.concat([s, Q_seq[:, i, :]], axis=1)
            s, p = self.step(s, p, Q_seq[:, i, :])
            # s = tf.transpose(tf.squeeze(s, axis=2))

            self.outputs = self.outputs.write(i+1, tf.concat([s[..., :-1], p[..., tf.newaxis], s[..., -1, tf.newaxis]], axis=1))
